      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
            <button
              onClick={() => { setMode('google'); setError(''); setMessage('') }}
              style={{
                flex: 1,
                padding: '8px 12px',
                background: mode === 'google' ? 'var(--color-accent)' : 'var(--bg-elevated)',
                color: mode === 'google' ? 'var(--bg-primary)' : 'var(--text-primary)',
                border: 'none',
                borderRadius: 'var(--radius-card)',
              ...
              onMouseEnter={e => { if (mode !== 'google') e.currentTarget.style.opacity = '0.8' }}
              onMouseLeave={e => { if (mode !== 'google') e.currentTarget.style.opacity = '1' }}
            >
              Google
            </button>
            <button
              ...
              style={{
                flex: 1,
                padding: '8px 12px',
                background: mode === 'email' ? 'var(--color-accent)' : 'var(--bg-elevated)',
                color: mode === 'email' ? 'var(--bg-primary)' : 'var(--text-primary)',
                border: 'none',
                borderRadius: 'var(--radius-card)',
                fontSize: 'var(--text-weight)',
                fontWeight: 500,
              ...
              onMouseEnter={e => { if (mode !== 'email') e.currentTarget.style.opacity = '0.8' }}
              onMouseLeave={e => { if (mode !== 'email') e.currentTarget.style.opacity = '1' }}
            >
              Email
            </button>
          </div>
          ...
          <p style={{ textAlign: 'center', marginTop: '24px', fontSize: 'var(--text-cat)', color: 'var(--text-tertiary)', letterSpacing: '0.02em' }}>
            {mode === 'google' ? 'Sign in with your Google account' : 'We\'ll send you a login link'}
          </p>
        </div>
      </div>
    </div>
